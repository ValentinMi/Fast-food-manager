...
const ProductCard = ({ user, product, actions, index, inOrderList }) => {
  ...
  const { removeProduct, refoundProduct } = productActions;

  const { addProductToOrder, removeProductFromOrder } = orderActions;
  ...
  function renderOrderUI() {
    return (
      <div className="order-box">
        ...
        <p>{price} €</p>
        {isAdmin ?
        <button className="btn btn-success" onClick={}>Ready</button>
        :
        <button
